...
function initMap() {
    ...
    map = new google.maps.Map(document.getElementById('map'), {
        center: {lat: 12.9716, lng: 77.5946}, //initially centered in the middle of the US, quickly replaced with current location
        zoom: 14
...
//    printLocations();
    ...
    var index = Math.floor(Math.random() * wayPoint.length) ;
    console.log(index);

    for(var i=0; i<1; i++) { //loop through the waypoints (skip start and end places)
//        console.log("Defining actualWaypoint[" + i + "]");
        actualWaypoints[i] = { //subtract 2 to fill this array starting at [0]
            location: wayPoint[index].geometry.location, //latlng object
            stopover: true
        }
    }
